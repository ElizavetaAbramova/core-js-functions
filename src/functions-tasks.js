...
function getPolynom(...args) {
  ...
  return (x) => {
    ...
    for (let i = 0; i < argsLength; i += 1) {
      result += args[i] * x ** (argsLength - 1 - i);
    }
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    logFunc(`${func.name}(${arrayArgs}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${arrayArgs}) ends`);
    return result;
  };
}
